...
type UploadReq =
  | { files?: number; token?: string }
  | { items?: UploadItemReq[]; token?: string };
...
function pickExt(name?: string | null) {
  const m = name?.match(/\.[a-zA-Z0-9]+$/);
  return m ? m[0].toLowerCase() : "";
}
function coerceContentType(ct?: string | null) {
  const v = (ct || "").trim().toLowerCase();
  // Allow images by default; fall back if unknown
...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
  try {
    ...
    if (!token) {
      return json({ ok: false, message: "Missing Turnstile token" }, { status: 400 });
    }
    if (!env.TURNSTILE_SECRET) {
      return json({ ok: false, message: "TURNSTILE_SECRET missing" }, { status: 500 });
    }

    /* --------- Verify Turnstile BEFORE issuing presigned URLs --------- */
      ...
      return json(
        {
          ok: false,
          message:
            "Missing env: R2_ACCOUNT_ID, R2_BUCKET, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY",
        },
        { status: 500 },
      );
    }
    ...
    for (const it of requestedItems) {
      ...
      const ext = pickExt(rawName);
      const key = `gallery/tmp/${crypto.randomUUID()}-${rawName}${ext ? "" : ""}`;
      const contentType = coerceContentType(it.contentType);

      // IMPORTANT: include ContentType here so the client must send the same header
