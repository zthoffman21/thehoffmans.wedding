...
export const onRequestPost: PagesFunction<Env> = async ({ env, params, request }) => {
  ...
  try {
    ...
    if (missing.length) {
      return json({ error: "Member(s) not in party", details: { partyId: party.id, missing } }, 400);
    }
    ...
    try {
      ...
      // Optional: upsert contact info on party
      if (parsed.data.contact && (parsed.data.contact.email || parsed.data.contact.phone)) {
        await env.DB
          .prepare(
            `UPDATE parties
          ...
          .bind(parsed.data.contact.email ?? null, parsed.data.contact.phone ?? null, party.id)
          .run();
      }

      // Upsert per-member current attendance
      ...
      if (idemKey) {
        await env.DB
          .prepare(`INSERT OR IGNORE INTO idempotency (key, submission_id) VALUES (?, ?)`)
