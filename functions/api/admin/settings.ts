...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
  ...
  const body = (await request.json().catch(() => ({}))) as Partial<{
    ...
    email_admin_on_uploads: "0" | "1" | 0 | 1 | boolean;
  }>;
  ...
  if (body.email_admin_on_uploads !== undefined) {
    const v =
      body.email_admin_on_uploads === true ||
      body.email_admin_on_uploads === 1 ||
      body.email_admin_on_uploads === "1"
        ? "1"
        : "0";
    entries.push(["email_admin_on_uploads", v]);
  }

  if (entries.length === 0) {
    return json({ ok: false, message: "No changes" }, { status: 400 });
  }
...
export const onRequestGet: PagesFunction<Env> = async ({ env }) => {
  const KEYS = [
    ...
    "purge_rejected_uploads",
    "email_admin_on_uploads",
  ] as const;
  ...
  const settings = {
    ...
    email_admin_on_uploads: (map["email_admin_on_uploads"] ?? "0") === "1",
  };
