...
export default function GalleryUpload() {
  ...
  useEffect(() => {
    ...
    async function loadTurnstile(timeoutMs = 7000) {
      ...
      if (!document.querySelector(`script[src="${src}"]`)) {
        ...
        s.defer = true;
        s.onerror = () => {
          if (!cancelled) setTsError("Could not load human-check (network/script blocked).");
        };
        document.head.appendChild(s);
      }

      // Wait until window.turnstile appears (or timeout)
      const start = Date.now();
      while (!window.turnstile && Date.now() - start < timeoutMs) {
    ...
    (async () => {
      ...
      if (!ok) {
        setTsError(
          "Human-check blocked. Disable ad-blockers for this page or allow challenges.cloudflare.com."
        );
        return;
      }

      // Render only if we have both the element and the API
      if (!tsElRef.current || !window.turnstile) return;
      ...
      const doRender = () => {
        try {
          ...
          console.error(e);
          setTsError("Human-check failed to initialize.");
        }
      };

      if (window.turnstile?.ready) window.turnstile.ready(doRender);
      else doRender();
    })();
    ...
    <section className="container-px py-8 max-w-xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button
          ...
          title={!tsLoaded ? "Human-check is loading…" : tsError ? "Human-check blocked" : ""}
        >
