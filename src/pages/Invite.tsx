    ...
    return (
        ...
        "
            ...
            >
                ...
                {/* RIGHT: vertically centered, left-aligned address; larger stamp; fewer ruled lines */}
                <div className="absolute left-[55%] right-5 top-5 bottom-24 sm:bottom-20 pl-5 grid">
                    {/* Vertically center in the panel, align left */}
                    <div className="relative self-center justify-self-start w-full">
                        {/* Short stack of faint ruled lines behind the text only */}
                        <div
                            aria-hidden="true"
                    ...
                    {/* Stamp + Postmark stay anchored top-right */}
                ...
                <div className="absolute inset-x-0 bottom-0 h-24 sm:h-36 border-t border-neutral-300/70 px-4 grid place-items-center">
                    ...
                    <div className="relative flex flex-col items-center text-center gap-2">
                        ...
                        <PerforatedButton to="/" variant="paper">
                            View Details
                        </PerforatedButton>

                        {/* faint perforation hint at the very bottom (ties into stamp vibe) */}
                        <div
                            aria-hidden
    ...
    return (
        ...
        >
            ...
            <g mask="url(#scallopMask)" filter="url(#softShadow)">
                ...
                {/* Detailed tulip (same as your latest) */}
                <g transform={`translate(${W / 2}, ${H / 2 + 4})`}>
                    ...
                    />
                    <path
                        d="M 0 -22 C 0 -18, 0 -12, 0 -8"
                        stroke="rgba(255,255,255,0.55)"
                        strokeWidth="0.8"
                    />
                    <path
                        d="M -6 -14 C -4 -12, -3 -10, -2 -8"
...
function Tape({ className = "" }: { className?: string }) {
    // Size with Tailwind (e.g., h-8 w-32) from the parent, rotate/position via className
    // The SVG has zig-zag ends on left/right edges.
    return (
        <svg
            className={`absolute ${className}`}
            viewBox="0 0 320 80" // logical size; scales with CSS h-/w-
        ...
        >
            <defs>
                ...
                {/* Combine gradient + stripes */}
                <mask id="tapeMask">
                    {/* Full opaque mask – we’ll clip with the zig-zag path itself */}
            ...
            {/* Sawtooth polygon:
          - Top/bottom are straight
          - Left/right have teeth (8px deep, 10px pitch).
          Feel free to tweak toothDepth / pitch by editing the points.
      */}
            ...
            M 0 0
            L 16 0
            L 12 8  L 16 16  L 12 24  L 16 32  L 12 40  L 16 48  L 12 56  L 16 64  L 12 72  L 16 80
            L 304 80
            L 308 72 L 304 64 L 308 56 L 304 48 L 308 40 L 304 32 L 308 24 L 304 16 L 308 8
            L 304 0
            Z
          "
                    fill="url(#tapeShade)"
                    ...
                    strokeWidth="1"
                />
                {/* Overlay stripes to simulate washi texture */}
                <path
                    d="
            M 0 0
            L 16 0
...
function Postmark() {
    // Circular cancellation mark overlapping the stamp
    return (
        <svg
...
function PerforatedButton({
    ...
    const palette = {
        paper: {
            ...
            inner1: "rgba(0,0,0,0.14)",
            inner2: "rgba(0,0,0,0.08)",
            arrow: "#D85A54",
        },
        sage: {
            bg: "#EAF0E2",
            text: "#2F332F",
            border: "rgba(56,68,52,0.22)",
            inner1: "rgba(56,68,52,0.14)",
            inner2: "rgba(56,68,52,0.10)",
            arrow: "#7FA466",
        },
        terracotta: {
            bg: "#F3E3DE",
            text: "#3B2E2B",
            border: "rgba(66,38,33,0.24)",
            inner1: "rgba(66,38,33,0.14)",
            inner2: "rgba(66,38,33,0.10)",
            arrow: "#C05C4F",
        },
        ink: {
            bg: "#F1EEE7",
            ...
            inner1: "rgba(0,0,0,0.16)",
            inner2: "rgba(0,0,0,0.10)",
            arrow: "#6B625E",
        },
    }[variant];
    ...
    return (
            ...
            style={{
                ...
                WebkitMaskImage: [
                    `radial-gradient(circle, transparent ${
                        tooth - 1
                    }px, black ${tooth}px) top / 100% ${tooth * 2}px repeat-x`,
                    `radial-gradient(circle, transparent ${
                        tooth - 1
                    }px, black ${tooth}px) bottom / 100% ${tooth * 2}px repeat-x`,
                    `radial-gradient(circle, transparent ${tooth - 1}px, black ${tooth}px) left / ${
                        tooth * 2
                    }px 100% repeat-y`,
                    `radial-gradient(circle, transparent ${
                        tooth - 1
                    }px, black ${tooth}px) right / ${tooth * 2}px 100% repeat-y`,
        ...
        >
            {/* double printed inner border like a stamp frame */}
            <span
                aria-hidden
                className="pointer-events-none absolute inset-1 rounded-[2px]"
                style={{ border: `1px solid ${palette.inner1}` }}
            />
            <span
                aria-hidden
                className="pointer-events-none absolute inset-[6px] rounded-[2px]"
                style={{ border: `1px solid ${palette.inner2}` }}
            />
